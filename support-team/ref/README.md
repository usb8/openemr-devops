1. Compose and deploy new docker images
```shell
docker compose up -d --build
```

2. Verify that Kafka connect is running
```shell
curl -s http://localhost:8083/ | jq # should be ready after 1 min
```

3. Register Connector via Kafka Connect API; JDBC
```shell
curl -i -X POST -H "Accept:application/json" -H "Content-Type:application/json" \
--data @debezium-mysql-1.json http://localhost:8083/connectors
...
```shell
curl -X GET http://localhost:8083/connectors/mariadb-connector-1/status # And verify the status of it
